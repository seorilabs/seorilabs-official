		...
		sections: [
			{
				...
				body: [
					...
					'앱은 기존 구매 권한을 확인하고 복원하기 위해 앱 실행 시 Firebase 익명 설치 사용자 ID를 생성합니다. 인앱결제를 선택한 경우 마켓·상품 ID, 구매 또는 주문의 거래 참조값과 토큰, 권한·환불 상태 및 처리 시각을 구매 검증과 복원 원장에 저장합니다. 결제 카드번호, 은행계좌, 이용자의 Google 계정 이름·이메일은 Seori Labs가 수집하지 않습니다.',
			...
			},
			{
				...
				body: [
					...
					'설치 사용자 ID와 구매 데이터는 유료 권한 확인·복원, 구매 검증, 환불 반영, 중복 및 부정 거래 방지, 법적 의무 준수를 위해 처리합니다.',
			...
			},
			{
				...
				body: [
					...
					'Google Firebase, Google Analytics 및 Google Cloud는 분석, 익명 인증, 구매 검증 원장과 서버 운영을 위해 Seori Labs의 서비스 제공자로서 데이터를 처리합니다. Google Play는 결제와 환불을 처리합니다.',
		...
		sections: [
			{
				...
				body: [
					...
					'The App creates a Firebase anonymous installation user ID at startup to check and restore existing purchase entitlements. If you choose an in-app purchase, we store market and product IDs, transaction references or tokens, entitlement and refund status, and processing timestamps in our purchase-validation and restoration ledger. Seori Labs does not collect payment card or bank details, or the name or email on your Google account.',
			...
			},
			{
				...
				body: [
					...
					'The installation user ID and purchase data are processed to check and restore paid entitlements, validate purchases, apply refunds, prevent duplicate or fraudulent transactions, and meet legal obligations.',
			...
			},
			{
				...
				body: [
					...
					'Google Firebase, Google Analytics, and Google Cloud process data as service providers for analytics, anonymous authentication, the purchase ledger, and server operations. Google Play processes payments and refunds.',
